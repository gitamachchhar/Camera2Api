...
public class RangeSeekBarView extends View {
    ...
    public void setThumbPosition(MediaData mData) {
        fromUser = false;
        setThumbPos(0, mData.getmThumb1Pos());
        setThumbPos(1, mData.getmThumb2Pos());
    }
    ...
    public float getThumbPos(int pos) {
       return mThumbs.get(pos).getPos();
    }
    ...
    private void drawThumbs(@NonNull Canvas canvas) {
        ...
        if (!mThumbs.isEmpty()) {
            for (Thumb th : mThumbs) {
                if (th.getIndex() == 0) {
                    canvas.drawBitmap(th.getBitmap(), th.getPos() + getPaddingLeft(), getPaddingTop() + mHeightTimeLine, p);
                } else {
                    canvas.drawBitmap(th.getBitmap(), th.getPos() - getPaddingRight(), getPaddingTop() + mHeightTimeLine, p);
                }
            }
        }
    }
